...
import type { TableProps, ExtendedColumnDef, ColumnGroup } from "./types";
import {
	Table as TableUI,
	TableHeader,
	TableBody,
...
/**
 * Main Table component for displaying data in a table format
 * Built on top of @tanstack/react-table and shadcn/ui
 */
export function Table<TData>({
	...
	// Priority: explicit prop > auto-detect based on totalRows
	// Manual pagination is used for server-side pagination
	// Automatic pagination is used for client-side pagination
	...
	const handleScroll = useCallback(
		...
		[isLoadOnScroll, onLoadMore, hasMore, isLoading],
	);
				...
				{table.getRowModel().rows.map((row, rowIndex) => {
					...
					return (
						<>
							<TableRow
								key={row.id}
								className={cn(rowClassName, dynamicRowClassName)}
							>
